...
# -------------------------------
#Petrel/remote backend stripper
#-------------------------------

def _remove_backend_args_recursively(obj):
    """Remove backend_args / Petrel backend blocks anywhere in the cfg tree."""
    if isinstance(obj, dict):
        # Drop generic backend args
        obj.pop('backend_args', None)
        obj.pop('file_client_args', None)  # legacy

        # If a node explicitly declares a Petrel backend/type, neutralize it
        t = obj.get('type')
        if isinstance(t, str) and t.lower().startswith('petrel'):
            obj.clear()  # safest: clear the block so nothing remote remains
            return

        # Recurse
        for v in obj.values():
            _remove_backend_args_recursively(v)

    elif isinstance(obj, (list, tuple)):
        for v in obj:
            _remove_backend_args_recursively(v)

def _rewrite_remote_paths(obj, repl=lambda s: s.replace('petrel://', '')):
    """Replace petrel:// URLs with local paths (default: strip the scheme)."""
    keys_like_paths = {'data_root','ann_file','img_prefix','img_dir','img_path','root','prefix'}
    if isinstance(obj, dict):
        for k, v in list(obj.items()):
            if k in keys_like_paths and isinstance(v, str) and v.startswith('petrel://'):
                obj[k] = repl(v)
            else:
                _rewrite_remote_paths(v, repl)
    elif isinstance(obj, (list, tuple)):
        for v in obj:
            _rewrite_remote_paths(v, repl)

def _strip_remote_backends(cfg: Config):
    """Top-level sanitizer for MMDet/MMEngine configs—force local I/O only."""
    # Global backend
    if hasattr(cfg, 'backend_args'):
        cfg.backend_args = None

    if hasattr(cfg, 'file_client_args'):
        cfg.file_client_args = None
    
    # common top-level nodes
    for key in ['train_dataloader','val_dataloader','test_dataloader',
                'train_dataset','val_dataset','test_dataset',
                'dataset','data','default_hooks','visualizer']:
        if hasattr(cfg, key):
            node = getattr(cfg, key)
            _remove_backend_args_recursively(node)
            _rewrite_remote_paths(node)    

    for key in ['train_dataloader','val_dataloader','test_dataloader']:
        if hasattr(cfg, key):
            dl = getattr(cfg, key)
            ds = dl.get('dataset') if isinstance(dl, dict) else None
            pipe = ds.get('pipeline') if isinstance(ds, dict) else None
            if isinstance(pipe, list):
                for t in pipe:
                    _remove_backend_args_recursively(t)
                    _rewrite_remote_paths(t)

    # final sweep on the whole dict
    _remove_backend_args_recursively(cfg._cfg_dict)
    _rewrite_remote_paths(cfg._cfg_dict)
...
def load_models(args):
    ...
    if args.model_config:
        config_path = Config.fromfile(args.model_config)
    else:
        config_path = _try_build_cfg_from_ckpt(ckpt_path)  # optional helper if you implemented it
        if config_path is None:
            raise SystemExit("Config is required: provide --model-config or use a checkpoint with embedded config.")

    # 2) >>> STRIP ANY REMOTE/PETREL BACKENDS HERE <<<
    _strip_remote_backends(config_path)

    # Optional: if test_dataloader carries hard petrel paths for ann_file/data_root,
    # nuke them to innocuous local values (dataset is only built for metainfo at init time)
    #td = getattr(config_path, 'test_dataloader', None)
    #if isinstance(td, dict) and 'dataset' in td and isinstance(td['dataset'], dict):
    #    ds = td['dataset']
    #    for k in ('data_root','ann_file','img_prefix','img_dir'):
    #        if k in ds and isinstance(ds[k], str) and ds[k].startswith('petrel://'):
    #            ds[k] = ds[k].replace('petrel://', '')
    #    ds.setdefault('metainfo', {})  # ensure metainfo key exists (helps avoid building remote)
        # You can inject known classes here if you like:
        # ds['metainfo']['classes'] = ('person','bicycle', ...)

    classes = CLASSNAMES  # however you obtained them earlier
    if not classes:
    ...
    # 2) If test_dataloader exists, nuke any remote-y fields
    td = getattr(config_path, 'test_dataloader', None)
    if isinstance(td, dict):
      ds = td.get('dataset')
      if isinstance(ds, dict):
        # strip any remote roots
        ds['data_root'] = ''
        ds['ann_file'] = None
        # common alias used in 3.x for images
        if isinstance(ds.get('data_prefix'), dict):
            for k, v in list(ds['data_prefix'].items()):
                if isinstance(v, str):
                    ds['data_prefix'][k] = ''  # make local/empty
        ds.setdefault('metainfo', {})
        ds['metainfo']['classes'] = tuple(classes)

      # also ensure sampler/dataloader bits won’t choke without dataset files
      td['batch_size'] = 1
      td['num_workers'] = 0
    

    from pprint import pprint
    print("[debug] backend_args:", getattr(config_path, 'backend_args', None))
    print("[debug] test_dataloader:")
    pprint(config_path.test_dataloader if hasattr(config_path, 'test_dataloader') else None)
